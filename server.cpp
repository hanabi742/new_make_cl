        ...
        while (true)
        {
            ...
            switch (static_cast<PacketType>(packet->type))
            {
                ...
                case PKT_REQ_UPLOAD_END:
                {
                    ...
                    if (storage.moveFileToFinal(packet->user_pk, packet->file_pk, current_original_name, current_file_size, &dummy_pk))
                    {
                        // 2. 최종 저장된 물리적 서버 경로
...
int main()
{
    ...
    while (true)
    {
        ...
        // 스레드 분리 (Detach) - 클라이언트와 1:1로 대화 시작
        // 모든 매니저 객체는 참조(ref)로 넘겨서 공유함
